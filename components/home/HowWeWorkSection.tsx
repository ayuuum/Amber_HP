...
const principleKeys = ['field', 'build', 'integrate', 'learn'] as const

export default function HowWeWorkSection() {
  const t = useMessages().home.howWeWork
        ...
        <ul className="grid gap-6 md:grid-cols-2">
          {howWeWorkPrinciples.map((item, index) => (
            <li key={item.enTitle} className="border-t border-sequoia-black/10 pt-5">
              ...
              <p className="text-sm leading-relaxed text-secondary md:text-base">{t.principles[principleKeys[index]]}</p>
            </li>
          ))}
        </ul>
      </div>
