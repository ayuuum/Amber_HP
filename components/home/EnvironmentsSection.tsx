    ...
    >
      <div className="home-container">
        <div className="mb-10 max-w-3xl md:mb-14">
          ...
          <p className="home-body max-w-2xl">主要なAIプラットフォームと既存システムへの対応です。</p>
        </div>
        ...
        <ul className="space-y-6">
          {environments.map((env) => {
            ...
            return (
              <li
                key={env.id}
                className="grid gap-8 bg-white p-6 md:grid-cols-[minmax(0,0.41fr)_minmax(0,0.59fr)] md:items-center md:gap-10 md:p-8"
              >
                <div className="flex flex-col items-center justify-center gap-4 text-center" aria-label={env.title}>
                  {showTitle ? (
                  ...
                  {logo ? (
                    <Image src={logo.src} alt={logo.alt} width={196} height={92} className={logo.className} />
                  ) : null}
                </div>
